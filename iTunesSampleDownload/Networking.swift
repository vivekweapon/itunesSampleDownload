            ...
            dataTask = downloadSession.dataTask(with: url, completionHandler: { (data, response, error) in
                ...
                else if let data = data,let response = response as? HTTPURLResponse,response.statusCode == 200
                {
                    ...
                    for sample in resultsArray! {
                        ...
                        let itunesSample = MusicSample(trackName: sample["trackName"] as! String, artistName: sample["artistName"] as! String, previewUrl: sample["previewUrl"] as! String, index: 1)
